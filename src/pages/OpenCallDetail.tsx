...
const OpenCallDetail = () => {
  ...
  const { data: openCall, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      console.log('Fetching open call details for ID:', callId);
  ...
  const { data: submissionCount } = useQuery({
    queryKey: ['submission-count', callId],
    queryFn: async () => {
      if (!callId || !isValidUUID(callId)) return 0;

      console.log('Fetching submission count for open call:', callId);
    ...
    enabled: !!callId && isValidUUID(callId || ''),
  });
  ...
  const handleSubmit = () => {
    ...
    if (callId && isValidUUID(callId)) {
      navigate(`/open-calls/${callId}/submit`);
    } else {
